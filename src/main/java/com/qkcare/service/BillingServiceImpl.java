...
public class BillingServiceImpl extends GenericServiceImpl implements BillingService {
	...
	public PatientService save(PatientService patientService) {
		...
		Pair<String, Long> itemLabelAndId = this.getItemLabelAndId(patientService.getVisit(),
				patientService.getAdmission());
	...
	public PatientPackage save(PatientPackage patientPackage) {
		...
		Pair<String, Long> itemLabelAndId = this.getItemLabelAndId(patientPackage.getVisit(),
				patientPackage.getAdmission());
	...
	public PatientSaleProduct save(PatientSaleProduct patientSaleProduct) {
		...
		Pair<String, Long> itemLabelAndId = this.getItemLabelAndId(patientSaleProduct.getVisit(),
				patientSaleProduct.getAdmission());
	...
	public Investigation save(Investigation investigation) {
		...
		Pair<String, Long> itemLabelAndId = this.getItemLabelAndId(investigation.getVisit(),
				investigation.getAdmission());
	...
	private void removeBillServiceFromBill(Long originServiceId, String originService) {
		...
		paramTupleList
				.add(Quartet.with("e." + originService + ".id = ", "originServiceId", originServiceId + "", "Long"));
	...
	public Double getPatientBillAmount(Patient patient) {
		...
		try {
			List<Quartet<String, String, String, String>> paramTupleList = new ArrayList<Quartet<String, String, String, String>>();
			String queryStr = "SELECT SUM(DUE) DUE FROM ( "
					+ "SELECT SUM(DUE) DUE FROM BILL B INNER JOIN VISIT V ON (B.VISIT_ID =V.VISIT_ID AND  V.PATIENT_ID ="+patient.getId()+") "
					+ "UNION "
					+ "SELECT SUM(DUE) DUE FROM BILL B INNER  JOIN ADMISSION A ON (B.ADMISSION_ID = A.ADMISSION_ID AND  A.PATIENT_ID ="+patient.getId()+")) T ";
			List<Double> list = (List) genericService.getNativeByCriteria(queryStr, paramTupleList, null, null);
